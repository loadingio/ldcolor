...
parse = do
  ...
  all: (o) ->
    if typeof(o) == \object =>
      ...
      else if o.value => parse.all(o.value)
      else o
...
conv = do
  num2rgb: (v) -> {r: v .>>. 16, g: (v .>>. 8) .&. 0xff, b: v .&. 0xff, a: 1}
  _hsl2rgb: (h, m1, m2) ->
  ...
  hcl2lab: (v) ->
    ...
    h = v.h * Math.PI / 180
    return {"@l": v.l, "@a": Math.cos(h) * v.c, "@b": Math.sin(h) * v.c, a: v.a}
...
# deprecated ldColor ( left for backward support ). use ldcolor from now.
else window.ldColor = window.ldcolor = ldcolor
